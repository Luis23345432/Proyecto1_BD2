...
def insert_record(user_id: str, db_name: str, table_name: str, payload: RecordInsert):
...
@router.get("")
def list_records(user_id: str, db_name: str, table_name: str, limit: int = Query(100, ge=1, le=1000), offset: int = Query(0, ge=0)):
    engine = DatabaseEngine(os.path.dirname(os.path.dirname(__file__)))
    db = engine.get_database(user_id, db_name)
    if db is None:
        raise HTTPException(status_code=404, detail="Database not found")
...
def spatial_range(user_id: str, db_name: str, table_name: str, payload: SpatialRange):
    engine = DatabaseEngine(os.path.dirname(os.path.dirname(__file__)))
    db = engine.get_database(user_id, db_name)
    if db is None:
        raise HTTPException(status_code=404, detail="Database not found")
...
@router.post("/knn")
def spatial_knn(user_id: str, db_name: str, table_name: str, payload: SpatialKNN):
    engine = DatabaseEngine(os.path.dirname(os.path.dirname(__file__)))
    db = engine.get_database(user_id, db_name)
    if db is None:
        raise HTTPException(status_code=404, detail="Database not found")
